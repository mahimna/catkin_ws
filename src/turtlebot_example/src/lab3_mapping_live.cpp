...
#define NUM_EDGES	50
#define OFFSET_X	1.14f
#define	OFFSET_Y	4.0f
#define LOOKAHEAD	0.3f
...
void pose_callback(const geometry_msgs::PoseWithCovarianceStamped & msg)
{
  ...
  ipsX = msg.pose.pose.position.x;
  ipsY = msg.pose.pose.position.y;
  ipsYaw = tf::getYaw(msg.pose.pose.orientation);
  ...
  points.id = 1;   //each curve must have a unique id or you will over$
...
void removeCollisionNodes()
{
  ...
  ROS_INFO("%f %f", nodesX[count-1], nodesY[count-1]);

  for (int i = 0; i < NUM_WAYPOINTS; i++)
  {
    nodesX[count] = waypointsX[i];
    nodesY[count++] = waypointsY[i];
    ROS_INFO("%f %f", nodesX[count-1], nodesY[count-1]);
  }
...
void carrot()
{
  ...
  goalX = closeX+cos(angle)*LOOKAHEAD;
  goalY = closeY+sin(angle)*LOOKAHEAD;
